...
namespace GUI
{
  public partial class FrmMain : Form
  {
    ...
    public void ChuyenForm(object sender, EventArgs e)// Code tại đây ! đây là hàm chuyển form khi click button
    {
      ...
      // Chức năng 0003 phân công
      //Phân công
      if (btnMenu.Name == "CN0003")
      {
        TruotPhai(pnlMenu, 0);
        TatTatCaTrenPnlMain(); //Chuyển về dạng miniMenu nhỏ
        urcQLPhanCong urcQLPhanCong = new urcQLPhanCong();
        urcQLPhanCong.bolNhanPhimHome = new urcQLPhanCong.NhanPhimHome(isClickedHome);
        if (KiemTraTonTaiCuaUrc(urcQLPhanCong, this) == true)
        {
          urcQLPhanCong.BringToFront();
          this.Controls.Add(urcQLPhanCong);
        }
      }
      ...
      /* CN0010 = Quản lý lịch làm việc */
      if (btnMenu.Name == "CN0010")
      {
        TruotPhai(pnlMenu, 0);
        TatTatCaTrenPnlMain(); //Chuyển về dạng miniMenu nhỏ
        urcQLLichLamViec urcQLLichLamViec = new urcQLLichLamViec();
        urcQLLichLamViec.bolNhanPhimHome = new urcQLLichLamViec.NhanPhimHome(isClickedHome);
        if (KiemTraTonTaiCuaUrc(urcQLLichLamViec, this) == true)
        {
          urcQLLichLamViec.BringToFront();
          this.Controls.Add(urcQLLichLamViec);
        }
      }

      // Chức năng 11 - Quản lý hóa đơn
      if (btnMenu.Name == "CN0011")
      {
        TruotPhai(pnlMenu, 0);
        TatTatCaTrenPnlMain(); //Chuyển về dạng miniMenu nhỏ
        urcQLDoanhThu urcQLDoanhThu = new urcQLDoanhThu();
        urcQLDoanhThu.bolNhanPhimHome = new urcQLDoanhThu.NhanPhimHome(isClickedHome);
        if (KiemTraTonTaiCuaUrc(urcQLDoanhThu, this) == true)
        {
          urcQLDoanhThu.BringToFront();
          this.Controls.Add(urcQLDoanhThu);
        }
      }
